...
def image_callback(data):
    ...
    crop_amount_width  = int(image.shape[1] - min(image.shape[1] / (height / settings.min_height), image.shape[1]))/2
    crop_amount_height = int(image.shape[0] - min(image.shape[0] / (height / settings.min_height), image.shape[0]))/2
    cropped = image[crop_amount_height:image.shape[0]-crop_amount_height,
                    crop_amount_width:image.shape[1]-crop_amount_width]

    resized_image = cv2.resize(cropped, settings.target_size, interpolation=cv2.INTER_LINEAR)
